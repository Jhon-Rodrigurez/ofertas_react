...
const SignupFormulario = ({errores, callback,
    ...
    uUrlLinkedin="", uUrlLogo="", uUrlCv="", editable})=> {
    ...
    const [urlLogo, setUrlLogo]= useState(uUrlLogo);
    const [urlCv, setUrlCv]= useState(uUrlCv);
    const [pdf, setPdfBytes] = useState(null);

    const setImagenFile = (e)=> {
        setUrlLogo(e.target.files[0])
    }
    ...
    const enviarFormulario = (e) => {
        e.preventDefault();
        (!editable) ? callback({nombre, email, idArea, idRol, password}) : callback({celular, descripcionExperiencia, urlLinkedin, urlLogo, urlCv})
    }
    ...
    return (
        <Form onSubmit={enviarFormulario}>
            ...
            {editable &&
                <Row>
                    <Col md="12" xs="12">
                        <Form.Group className="mt-3 mb-3" controlId="urlLogo">
                            <Form.Label className="fw-semibold fs-5">Foto de perfil *opcional*</Form.Label>
                            <Form.Control
                            type="file"
                            name="urlLogo"
